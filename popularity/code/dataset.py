...
def filter_user_item_limit(df, user_limit, item_limit):
    # Calculate the number of reviews per user and item
    user_counts = df['user'].value_counts()
    item_counts = df['item'].value_counts()

    # Filter users and items that meet the minimum number of reviews
    users_to_keep = user_counts[user_counts >= user_limit].index
    items_to_keep = item_counts[item_counts >= item_limit].index

    # Select data that contains only the filtered users and items
    filtered_df = df[df['user'].isin(users_to_keep) & df['item'].isin(items_to_keep)]

    # Set up for iterative filtering
...
def make_dataset(config: Config.Config):
    ...
    if all(os.path.exists(path) for path in dataset_paths):
    ...
    if 'Amazon' in main_path:    
        df = pd.read_csv(main_path + dataset, compression='gzip')
        columns_to_keep = {'user_id': 'user', 'parent_asin': 'item', 'timestamp': 'time', 'rating': 'rate'}
    ...
    if is_douban:
        ...
        necessary_columns = {'main_category': 'genre', 'average_rating': 'avg_rating', 'store': 'director', 'parent_asin': 'name'}
    ...
    def update_pop_history(item_pop, valid_start_time, max_time):
        # update popularity history and return valid length
    ...
    def append_dataset(dataset, item_idx, item_time_release, side_info, avg_rating, time_now, pop_history, pop_gt, valid_pop_len):
...
class Data(data.Dataset):
    ...
    def load_dataset(self):
        ...
        df = pd.read_csv(data_path, header=0, index_col=0,
                         dtype={'item': int, 'time_release': int, 'side_info': str, 'avg_rating': float,
    ...
    def __getitem__(self, idx):
        # ['item', 'time_release', 'side_info', 'avg_rating', 'time', 'pop_history', 'pop_gt', 'valid_pop_len']
        return [self.data['item'][idx], self.data['time_release'][idx], self.data['side_info'][idx], self.data['avg_rating'][idx],
