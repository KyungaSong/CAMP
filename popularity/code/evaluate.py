...
def m_NDCG(actual, predicted, topk):
    ...
    idcg = idcg_k(k)
    dcg_k = sum([int(predicted[j] in set(actual)) / math.log(j + 2, 2) for j in range(k)])
    res += dcg_k / idcg
    return res 
...
def compute_precision_recall(targets, predictions, k):
    ...
    precision = float(num_hit) / len(pred) if len(pred) > 0 else 0
    recall = float(num_hit) / len(targets) if len(targets) > 0 else 0

    # 1 if there's at least one hit, 0 otherwise
...
def pred_for_all_item(model, config:Config.Config):
    ...
    for i in item_list:
        if i in config.dict_side_info:
            ...
            print("type", type(config.dict_side_info[i]))
        else:
            side_info.append([[0]])
...
def calculate_hr_ndcg(model, config: Config.Config):
    print("config.ori_dataset: ", config.ori_dataset)
    douban_file = config.processed_path + '{}_tra_test.txt'.format(config.ori_dataset)
    ...
    if config.alpha > 0:
        valid_pop_len = torch.tensor(valid_pop_len)
        valid_pop_len = torch.clamp(valid_pop_len, min=None, max=config.alpha)
    ...
    for i in test_data.keys():
        ...
        for k in [5, 10]:
            ...
            _, _, r, _, n = compute_precision_recall(gt_now, prediction, k)
            list_r.append(r)
            list_n.append(n)
        HR.append(list_r)
        NDCG.append(list_n)
    ...
    return np.mean(HR, axis=0), np.mean(NDCG, axis=0), mse, pred.cpu().detach().numpy().tolist()