...
parser.add_argument("--embedding_dim", type=int, default=258,
...
def load_data(dataset_name):    
    ...
    if os.path.exists(f'{processed_path}/train_df_pop.pkl') and os.path.exists(f'{processed_path}/valid_df_pop.pkl') and os.path.exists(f'{processed_path}/test_df_pop.pkl') and config.data_preprocessed:
        ...
        try:
            ...
            # print(f'data: {len(df)}, num_users: {num_users}, num_items: {num_items}, num_cats: {num_cats}, num_stores: {num_stores}')
            ...
            end_time = time.time()
            # print(f"Dataset prepared in {end_time - start_time:.2f} seconds")
