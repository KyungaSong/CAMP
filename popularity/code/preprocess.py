...
def load_dataset(file_path, meta = False):
    with open(file_path, 'rb') as file:
        df = pickle.load(file) 
    if meta:
        necessary_columns = ['average_rating', 'rating_number', 'store', 'parent_asin', 'category']
        df = df[necessary_columns].rename(columns={'average_rating': 'avg_rating', 'parent_asin': 'item_id'})
        df['category'] = df['category'].apply(lambda x: x[1] if len(x) > 1 else (x[0] if len(x) == 1 else None))
    else:        
...
def preprocess_df(df, config):     
    ...
    additional_info = df.groupby('item_encoded')[['avg_rating', 'rating_number', 'cat_encoded', 'store_encoded']].first().reset_index()
    df_pop = df_pop.merge(additional_info, on='item_encoded', how='left')
    print("df_pop:\n", df_pop)
    
    print("pop history ex: \n", df_pop['pop_history'].iloc[10])
...
def create_datasets(train_df, valid_df, test_df):
    train_dataset = MakeDataset(
        ...
        train_df['release_time'], train_df['avg_rating'], train_df['rating_number'], train_df['cat_encoded'], train_df['store_encoded']
    )
    valid_dataset = MakeDataset(
        valid_df['item_encoded'], valid_df['pop_history'],
        valid_df['release_time'], valid_df['avg_rating'], valid_df['rating_number'], valid_df['cat_encoded'], valid_df['store_encoded']
    )
    test_dataset = MakeDataset(
        test_df['item_encoded'], test_df['pop_history'],
        test_df['release_time'], test_df['avg_rating'], test_df['rating_number'], test_df['cat_encoded'], test_df['store_encoded']
    )
